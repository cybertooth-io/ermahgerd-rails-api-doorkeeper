...
Doorkeeper.configure do
  # the ORM that doorkeeper will use (needs plugins)
  # --------------------------------------------------------------------------------------------------------------------
  orm :active_record
  ...
  # Throw DoorkeeperError should the authentication attempt fail, else return the User
  # --------------------------------------------------------------------------------------------------------------------
  resource_owner_from_credentials do
    user = User.find_by(email: params[:username]).try(:authenticate, params[:password])

    raise Doorkeeper::Errors::DoorkeeperError, I18n.t('doorkeeper.errors.messages.access_denied') if user.blank?
  ...
  # --------------------------------------------------------------------------------------------------------------------
  grant_flows %w[password] # TODO: add `implicit`
