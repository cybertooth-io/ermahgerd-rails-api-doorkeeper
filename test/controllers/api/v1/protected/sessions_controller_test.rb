    ...
    module Protected
      class SessionsControllerTest < ActionDispatch::IntegrationTest
        ...
        test 'when session is invalidated the access token is immediately unusable' do
          ...
          get api_v1_protected_users_url, headers: some_administrator_headers

          assert_response :ok

          # now using some_other_administrator's account, invalidate some_administrator's session
          patch invalidate_api_v1_protected_session_url(some_administrator_session.id), headers: @headers
